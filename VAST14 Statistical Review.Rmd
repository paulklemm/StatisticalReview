...
The boxplot shows that there are most likely error codes still in the `BMI` group, which are therefore removed.

```{r fig.width=10, fig.height=8}
ship_valid_backpain_bmi <- filter(.data = ship_valid_backpain, BMI < 90000)

ship_age_valid_backpain_bmi <- filter(.data = ship_age_valid_backpain, BMI < 90000)

par(mfrow=c(1,2), oma=c(1,1,2,1), mar=c(2,2,2,2))
boxplot(ship_valid_backpain_bmi$BMI ~ ship_valid_backpain_bmi$Rueckenschmerz_3Monate, main="back pain by BMI (mean is black dot)", xlab="back pain", ylab="BMI", col=rainbow(7))

boxplot(ship_age_valid_backpain_bmi$BMI ~ ship_age_valid_backpain_bmi$Rueckenschmerz_3Monate, main="back pain by BMI (mean is black dot)", xlab="back pain", ylab="BMI", col=rainbow(7))

summary(aov(formula = BMI~Rueckenschmerz_3Monate, data = ship_valid_backpain_bmi))

summary(aov(formula = BMI~Rueckenschmerz_3Monate, data = ship_age_valid_backpain_bmi))
```
